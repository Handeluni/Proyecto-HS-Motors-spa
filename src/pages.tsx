...
import { getVehicleById, getVehicles, submitQuote, submitTestDrive } from './services/vehicleService'
import type { FuelType, RequestStatus, Vehicle, VehicleCategory } from './types'
import { formatPrice } from './utils/format'

export function HomePage(){const [featured,setFeatured]=useState<Vehicle[]>([]);useEffect(()=>{getVehicles().then(items=>setFeatured(items.filter(v=>v.featured).slice(0,3)))},[]);return <><section className="hero"><div className="container hero-copy"><p className="eyebrow">HS MOTORS · DESDE 2026</p><h1>Elige el camino que <em>te mueve.</em></h1><p>Vehículos excepcionales y un servicio cercano para que disfrutes cada kilómetro con total confianza.</p><div className="hero-actions"><Link className="button" to="/vehicles">Explorar vehículos <ArrowRight/></Link><Link className="button ghost" to="/test-drive">Agendar prueba</Link></div></div></section><section className="section container"><div className="section-title"><div><p className="eyebrow">SELECCIÓN HS</p><h2>Vehículos destacados</h2></div><Link className="text-link" to="/vehicles">Ver catálogo completo <ArrowRight/></Link></div><div className="vehicle-grid">{featured.length?featured.map(v=><VehicleCard key={v.id} vehicle={v}/>):[1,2,3].map(n=><VehicleSkeleton key={n}/>)}</div></section><section className="band"><div className="container metrics"><div><strong>8+</strong><span>modelos disponibles</span></div><div><strong>6</strong><span>servicios especializados</span></div><div><strong>100%</strong><span>atención personalizada</span></div></div></section><section className="section container"><div className="section-title"><div><p className="eyebrow">MÁS QUE VEHÍCULOS</p><h2>Todo para avanzar</h2></div><Link className="text-link" to="/services">Conocer servicios <ArrowRight/></Link></div><div className="service-grid compact">{services.slice(0,3).map(s=><ServiceCard key={s.id} service={s}/>)}</div></section><section className="why"><div className="container why-grid"><div><p className="eyebrow">LA DIFERENCIA HS</p><h2>Te acompañamos antes, durante y después de decidir.</h2><p>Combinamos una selección rigurosa con asesoría humana. Sin presión, sin letras pequeñas.</p><Link className="button" to="/quote">Hablemos de tu próximo vehículo</Link></div><div className="checks"><p><CircleCheck/> Inventario cuidadosamente seleccionado</p><p><CircleCheck/> Equipo experto a tu disposición</p><p><CircleCheck/> Procesos transparentes y simples</p></div></div></section><section className="cta container"><CalendarDays/><div><h2>¿Listo para sentirlo en la ruta?</h2><p>Agenda una prueba de manejo sin compromiso.</p></div><Link className="button light-button" to="/test-drive">Agendar ahora <ArrowRight/></Link></section></>}
